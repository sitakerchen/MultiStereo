...
{
    for (int i = 0 ; i < m_ListTcpClient.length() ; ++ i) // traver through every established connections
    {
        ...
        IP_Port = tr("[%1:%2]:").arg(m_ListTcpClient[i]->peerAddress().toString().split("::ffff:")[1])\
        ...
        if (QString(buffer) == "file done") // file send successfully
        {
    ...
    for (int i = 0 ; i < m_ListTcpClient.length() ; ++ i)
    {
        if (m_ListTcpClient[i]->state() == QAbstractSocket::UnconnectedState)
        {
            ui->comboBox->removeItem(ui->comboBox->findText(tr("%1:%2")\
                                                            .arg(m_ListTcpClient[i]->peerAddress().toString().split("::ffff:")[1])\
                                     .arg(m_ListTcpClient[i]->peerPort())));
...
void tcpserver::on_pushButtonDisconnect_clicked()
{
    ...
    for (int i = 0 ; i < m_ListTcpClient.length() ; ++ i)
    {
        ...
        if (m_ListTcpClient[i]->state() == QAbstractSocket::UnconnectedState or m_ListTcpClient[i]->waitForDisconnected(1000))
        {
...
void tcpserver::on_pushButton_sendFile_clicked()
{
    ...
    QString head = QString("%1##%2").arg(m_outFile.fileName).arg(m_outFile.fileSize);
